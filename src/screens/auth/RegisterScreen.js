...
export default function RegisterScreen({ navigation }) {
  ...
  const onRegister = async () => {
    ...
    try {
      try { navigation.reset({ index: 0, routes: [{ name: 'MainTabs', params: { screen: 'Home' } }] }); } catch { navigation.navigate('Home'); }
    } catch { }
  };
    ...
    <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }} style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      <View style={{ marginTop: 12, alignItems: 'center' }}>
        <Text style={{ color: theme.colors.muted }}>
          ...
          <Text style={{ color: theme.colors.primary, fontWeight: '700' }} onPress={() => goToLogin(navigation)}>
            {t('login') || 'Login'}
          </Text>
        </Text>
      </View>
